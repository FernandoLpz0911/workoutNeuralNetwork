...
# Load the CSV data
df = pd.read_csv('FitNotes_Export_apr7.csv')

print("Exact columns Pandas sees:", df.columns.tolist())
df.columns = df.columns.str.strip()

# Clean the data and calculate basic metrics
# Filter out non-resistance exercises (like stretching/cardio)
...
# Create a mask that identifies the maximum weight lifted for each exercise on a given day
...
# Calculate Historical/Lag Features
# Sort by Exercise and Date to ensure chronological order
...
# Save the processed data for the ML Model and Hybrid Engine to use
workout_summary.to_csv('Processed_Workout_Data.csv', index=False)
print("\nData processing complete. Saved to Processed_Workout_Data.csv")